...
const PurchasesScreen = () => {
  ...
  const primaryColor = '#354024';
  const backgroundColor = '#ffffff';
  const accentColor = '#D69992'; // Optional accent color
  ...
  return (
    <View style={[styles.container, { backgroundColor }]}>
      ...
      <Text style={[styles.title, { color: primaryColor }]}>Your Purchases</Text>

      {purchaseHistory.length === 0 ? (
        <Text style={[styles.empty, { color: primaryColor }]}>
          No purchases yet.
        </Text>
      ) : (
        <FlatList
          ...
          renderItem={({ item: purchase, index }) => (
            <View
              style={[styles.purchaseBlock, { borderColor: backgroundColor }]}
              key={index}
            >
